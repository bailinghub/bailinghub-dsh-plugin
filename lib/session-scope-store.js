...
const SCHEMAS = new Set(['bailing.agent-session-scope.v1', 'bailing.agent-session-scope.v2'])
...
function storeError(code = 'SCOPE_STORE_UNAVAILABLE') {
  const error = new Error(code === 'SCOPE_STORE_CONFLICT'
    ? 'The session scope changed or is being saved. Read it again before retrying.'
    : 'The session scope store is unavailable. The scope was not safely confirmed.')
  error.code = code
  return error
}

function safeError(error) {
  return ['SCOPE_STORE_CONFLICT', 'SCOPE_STORE_UNAVAILABLE'].includes(error?.code)
    ? error : storeError()
}
...
// The coordinator owns the scope schema. This boundary additionally refuses
...
function assertRevision(current, expectedRevision) {
  if ((current?.revision ?? null) !== expectedRevision) throw storeError('SCOPE_STORE_CONFLICT')
}
...
function defaultDirectory() {
  ...
  return join(resolve(expanded), 'plugins', 'dsh-bailinghub', 'session-scopes')
}
...
async function acquireLock(path) {
  ...
  while (true) {
    ...
    try {
      ...
      if (performance.now() >= deadline) throw storeError('SCOPE_STORE_CONFLICT')
...
/** Durable non-secret scope sidecars. No user directory is touched until load/save. */
export function createFileSessionScopeStore({ directory } = {}) {
...
export function createMemorySessionScopeStore() {
