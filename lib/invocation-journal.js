...
const IDENTITY_FIELDS = ['id', 'scopeHash', 'scopeRevision', 'target', 'runId', 'turn', 'callId', 'tool', 'capabilityRevision', 'parameterHash']
...
export class InvocationJournal {
  ...
  async read(sessionId) {
    ...
    try {
      ...
      if (raw.schema !== SCHEMA || raw.sessionId !== sessionId || !Number.isSafeInteger(raw.revision) || raw.revision < 1 || raw.revision < seen ||
      ...
      const entries = raw.entries.map(checkedEntry)
      ...
      try {
        ...
        return { state: reason || unsavedRecords ? 'storage_error' : 'ready', ...(reason ? { reason } : {}),
          ...
          entries: record.entries.map(entry => ({ invocation_id: entry.id, authorization_ref: entry.target.authorizationRef,
            tool: entry.tool, original_run_id: entry.runId, last_known_state: entry.lastKnownState,
