...
const SCHEMAS = new Set(['bailing.agent-invocations.v1', 'bailing.agent-invocations.v2'])
...
function storeError(code = 'INVOCATION_STORE_UNAVAILABLE') {
  const error = new Error(code === 'INVOCATION_STORE_CONFLICT'
    ? 'The invocation record changed or is being saved. Read it again before retrying.'
    : 'The invocation store is unavailable. Recovery metadata was not safely confirmed.')
  error.code = code
  return error
}

function safeError(error) {
  return ['INVOCATION_STORE_CONFLICT', 'INVOCATION_STORE_UNAVAILABLE'].includes(error?.code)
    ? error : storeError()
}
...
function assertRevision(current, expectedRevision) {
  if ((current?.revision ?? null) !== expectedRevision) throw storeError('INVOCATION_STORE_CONFLICT')
}
...
function defaultDirectory() {
  ...
  return join(resolve(expanded), 'plugins', 'dsh-bailinghub', 'invocation-records')
}
...
async function acquireLock(path) {
  ...
  while (true) {
    ...
    try {
      ...
      if (performance.now() >= deadline) throw storeError('INVOCATION_STORE_CONFLICT')
...
/** Durable execution metadata only. No user directory is touched until load/save. */
export function createFileInvocationStore({ directory } = {}) {
