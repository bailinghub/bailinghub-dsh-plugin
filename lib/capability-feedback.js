...
const ID = /^[0-9a-f]{64}$/
const CATEGORIES = new Set(['tool_not_loaded', 'capability_changed', 'transport_unavailable', 'authorization_unavailable', 'unsupported', 'invocation_outcome_unknown', 'unknown_failure', 'cancelled', 'invalid_request', 'storage_error', 'invocation_recovery_unavailable', 'task_control'])
const ACTIONS = new Set(['rediscover', 'retry_discovery', 'restore_scope', 'reauthorize', 'check_compatibility', 'resume_original', 'inspect_original', 'restore_invocations', 'restore_task', 'inspect_task', 'none'])
const ORIGINS = new Set(['core', 'sdk', 'mcp', 'dsh', 'host'])
const CODES = new Set([
  'TASK_DISPATCH_UNCERTAIN', 'TASK_NOT_FOUND', 'TASK_RUN_INACTIVE', 'TASK_INVALID_INPUT',
  'TASK_REQUIRED', 'TASK_UNSUPPORTED', 'TASK_SCOPE_BLOCKED', 'TASK_MEMBER_MISMATCH', 'TASK_BINDING_CONFLICT', 'TASK_PAUSED', 'TASK_CANCELLED', 'TASK_EXPIRED', 'TASK_WRITE_BUDGET_EXHAUSTED', 'TASK_CONCURRENCY_EXHAUSTED', 'TASK_TOOL_NOT_ALLOWED', 'TASK_REVISION_CONFLICT', 'TASK_RECORD_INVALID', 'TASK_UNAVAILABLE', 'TASK_STORE_CONFLICT', 'TASK_STORE_UNAVAILABLE',
  ...
  'SCOPE_STORE_UNAVAILABLE', 'SCOPE_STORE_CONFLICT', 'SESSION_SCOPE_CONFLICT',
  'INVALID_SCOPE_SNAPSHOT', 'INVALID_SESSION_ID', 'SESSION_LOCAL_STORAGE_ERROR', 'SESSION_RECOVERY_GAP',
])

const MESSAGES = {
  task_control: 'The original task does not currently permit this operation. Inspect its task state; do not rediscover tools, change authorization, replace the invocation or create a new task to bypass the limit.',
...
export function describeFailure(error, context = {}) {
  ...
  const missingOriginal = (['resume', 'inspect'].includes(operation) && isInvocationNotFound(error)) ||
  ...
  if (!source) {
    if (['TASK_DISPATCH_UNCERTAIN', 'TASK_UNAVAILABLE'].includes(code) && ['invoke', 'resume'].includes(operation)) { category = 'invocation_outcome_unknown'; next = 'inspect_original'; dispatch = 'unknown' }
    else if (code.startsWith('TASK_')) { category = code.startsWith('TASK_STORE_') ? 'storage_error' : code === 'TASK_UNSUPPORTED' ? 'unsupported' : 'task_control'; next = category === 'storage_error' ? 'restore_task' : category === 'unsupported' ? 'check_compatibility' : 'inspect_task'; dispatch = 'not_dispatched' }
    else if (error?.name === 'AbortError' || code === 'agent_request_cancelled') category = 'cancelled'
  ...
  if (error?.disposition === 'accepted_unknown' || source?.disposition === 'accepted_unknown' || category === 'invocation_outcome_unknown') {
    category = 'invocation_outcome_unknown'; dispatch = dispatch === 'not_dispatched' ? 'unknown' : dispatch; next = source?.next_action === 'inspect_original' || ['reconciliation_required', 'TASK_DISPATCH_UNCERTAIN', 'TASK_UNAVAILABLE'].includes(code) || !invocationId ? 'inspect_original' : 'resume_original'
  } else if (!source && operation === 'resume' && invocationId && !['cancelled', 'authorization_unavailable', 'unsupported', 'task_control', 'storage_error'].includes(category)) {
    next = ['transport_unavailable', 'unknown_failure'].includes(category) ? 'resume_original' : 'inspect_original'
  } else if (!source && operation === 'invoke' && invocationId && dispatch !== 'not_dispatched' && !['cancelled', 'authorization_unavailable', 'unsupported', 'capability_changed', 'task_control', 'storage_error'].includes(category)) {
  ...
  if (operation === 'task_coordinates') {
    // Coordinates never dispatch. Preserve the shared feedback envelope while
    // distinguishing local persistence from retryable read-only network probes.
    dispatch = 'not_dispatched'
    if (['SCOPE_STORE_UNAVAILABLE', 'SCOPE_STORE_CONFLICT'].includes(code)) { category = 'storage_error'; next = 'restore_scope' }
    else if (['SESSION_LOCAL_STORAGE_ERROR', 'SESSION_RECOVERY_GAP'].includes(code)) { category = 'storage_error'; next = 'none' }
    else if (['SESSION_SCOPE_CONFLICT', 'INVALID_SCOPE_SNAPSHOT'].includes(code)) { category = 'authorization_unavailable'; next = 'restore_scope' }
    else if (code === 'INVALID_SESSION_ID') { category = 'invalid_request'; next = 'none' }
    else if (['TASK_UNAVAILABLE', 'agent_runtime_unavailable', 'agent_invalid_response'].includes(code)) { category = 'transport_unavailable'; next = 'restore_scope' }
    else if (error?.statusCode === 429 || error?.statusCode >= 500) { category = 'transport_unavailable'; next = 'restore_scope' }
  }
  return {
    ...
    message: code.startsWith('TASK_STORE_') ? 'The local task association could not be saved or verified. Restore the original task binding before continuing; do not switch or create a replacement task.' : missingOriginal
