...
const ID = /^[0-9a-f]{64}$/
const CATEGORIES = new Set(['tool_not_loaded', 'capability_changed', 'transport_unavailable', 'authorization_unavailable', 'unsupported', 'invocation_outcome_unknown', 'unknown_failure', 'cancelled', 'invalid_request'])
const ACTIONS = new Set(['rediscover', 'retry_discovery', 'restore_scope', 'reauthorize', 'check_compatibility', 'resume_original', 'inspect_original', 'none'])
...
export function describeFailure(error, context = {}) {
  ...
  return {
    ...
    operation, dispatch,
    retryable: ['rediscover', 'retry_discovery', 'restore_scope', 'resume_original'].includes(next),
    next_action: next,
...
export const DISCOVERY_GUIDANCE = 'Capability search returns bounded ranked candidates, not an exhaustive matching list or pageable catalog. A search replaces that target’s candidates and recomputes the session tool set under its shared limit. Use currently loaded, valid tools directly when the selected target is clear; search again only when needed. not_loaded means not yet loaded, not no capability. Never use discovery to replace an uncertain invocation.'
