...
export class SessionScopeCoordinator {
  ...
  async set(sessionId, request) {
    ...
    try {
      ...
      entry.blocked = true
      // A durable tombstone prevents restart from resurrecting the old selection
      // if validating or saving its replacement fails.
  ...
  async restore(sessionId) {
    const entry = await this.load(this.entry(sessionId))
    if (entry.pending) { entry.blocked = true; entry.generation += 1 }
    if (!entry.record) { entry.blocked = true; entry.claimed = true }
