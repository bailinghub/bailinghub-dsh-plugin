...
function definitiveAuthorizationFailure(error) {
  return ['AUTHORIZATION_CHANGED', 'CROSS_SYSTEM_SCOPE_UNSUPPORTED'].includes(error?.code) ||
...
export class SessionScopeCoordinator {
  ...
  async revalidate(entry) {
    ...
    validation.promise = (async () => {
      try {
        ...
        await this.checkStored(entry)
        if (current()) entry.retryRequired = false
      } catch (error) {
  ...
  async resolve(connectionKeys, original) {
    ...
    for (const connectionKey of connectionKeys) {
      ...
      this.validateStatus(status, { connectionKey, workspace: binding.workspace, sessionId: saved?.sessionId })
