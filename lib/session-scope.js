...
import { projectSubjectDisplay, subjectDisplayLabel } from './subject-display.js'
...
export class SessionScopeCoordinator {
    ...
    if (!entry.loaded) {
      entry.loading ??= (async () => {
        try {
          ...
          entry.failureCode = error?.code === 'INVALID_SCOPE_SNAPSHOT' ? error.code : 'SCOPE_STORE_UNAVAILABLE'
        }
        entry.loaded = true
      })()
      await entry.loading
  ...
  view(entry) {
    ...
    return {
      ...
      authorizations: blocked ? [] : (record?.authorizations ?? []).map((item) => ({
        ...
        ...this.subjectDisplay(entry.sessionId, item.connectionKey),
  ...
  async checkStored(entry) {
    ...
    try {
      ...
      if (entry.record === original && entry.generation === generation) {
        entry.blocked = true
        entry.failureCode = ['SESSION_SCOPE_CONFLICT', 'SCOPE_STORE_CONFLICT', 'INVALID_SCOPE_SNAPSHOT'].includes(error?.code)
          ? error.code : 'SCOPE_STORE_UNAVAILABLE'
      }
    }
  }
  ...
  async revalidate(entry) {
    ...
    validation.promise = (async () => {
      try {
        const selected = await this.resolve(original.authorizations.map((item) => item.connectionKey), original)
        if (!current()) return
        // A network wait must not hide a concurrent durable scope change.
        await this.checkStored(entry)
        if (current()) {
          entry.subjectDisplays = selected.subjectDisplays
          entry.retryRequired = false; entry.failureCode = undefined
        }
      } catch (error) {
  ...
  async resolve(connectionKeys, original) {
    if (!connectionKeys.length) return { binding: null, authorizations: [], subjectDisplays: new Map() }
    ...
    const subjectDisplays = new Map()
    const sessions = new Set()
    for (const { connectionKey, candidate, saved } of selections) {
      ...
      // The freshly verified status belongs to this exact original key and
      // Session. Keep mutable display fields out of persisted identity/CAS data.
      const display = projectSubjectDisplay(status, this.sanitize)
      subjectDisplays.set(connectionKey, display)
      const label = subjectDisplayLabel(display)
    ...
    return { binding, authorizations, subjectDisplays }
  }
  ...
  async set(sessionId, request) {
    ...
    try {
      ...
      await this.write(entry, { state: 'ready', locked: false, binding: selected.binding, authorizations: selected.authorizations })
  ...
  // Read-only host projection: drafts are valid selections, not dispatch grants.
  // Retain object identity and generation across every await, including A -> B -> A.
  async verifiedSelection(sessionId) {
    const entry = await this.load(this.entry(sessionId))
    const record = entry.record
    const generation = entry.generation
    const settleValidation = async () => {
      // Another read can revalidate the same original group while this reader
      // waits for local I/O. Join that proof; never mistake its temporary closed
      // gate for a changed identity, nor ignore its eventual revocation result.
      while (entry.record === record && entry.generation === generation && this.isValidating(entry)) await entry.validation.promise
    }
    const assertCurrent = () => {
      if (entry.record !== record || entry.generation !== generation) throw scopeError('SESSION_SCOPE_CONFLICT')
      if (entry.pending || entry.blocked || entry.retryRequired || this.isValidating(entry) || record?.state === 'needs_selection') {
        throw scopeError(entry.failureCode ?? 'SESSION_SCOPE_UNAVAILABLE')
      }
    }
    await this.get(sessionId)
    await settleValidation()
    // An identity failure increments generation; preserve its authoritative code.
    if (entry.record === record && entry.generation !== generation && (entry.blocked || entry.retryRequired) && !entry.pending) {
      throw scopeError(entry.failureCode ?? 'SESSION_SCOPE_UNAVAILABLE')
    }
    assertCurrent()
    return { record: structuredClone(record), assertCurrent, check: async () => {
      await settleValidation()
      assertCurrent()
      if (record) await this.checkStored(entry)
      await settleValidation()
      assertCurrent()
    } }
  }

  subjectDisplay(sessionId, connectionKey) {
    return structuredClone(this.entry(sessionId).subjectDisplays?.get(connectionKey) ?? projectSubjectDisplay(null))
  }
  ...
  invalidate(sessionId, error) {
    const entry = this.entry(sessionId)
    entry.generation += 1
    const definitive = !error || definitiveAuthorizationFailure(error)
    entry.failureCode = definitive ? error?.code === 'CROSS_HUB_SCOPE_UNSUPPORTED' ? error.code : 'AUTHORIZATION_CHANGED'
      : error?.name === 'AgentAuthHttpError' && (error.statusCode === 429 || error.statusCode >= 500) ? 'agent_transport_unavailable'
      : error?.publicCode ?? error?.code ?? 'AUTHORIZATION_UNVERIFIED'
