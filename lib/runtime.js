...
const PUBLIC_USAGE_KEYS = new Set([
  'input_tokens',
  'cached_input_tokens',
  'output_tokens',
  'total_tokens',
...
function sumUsage(usages) {
  ...
  for (const usage of usages) {
    ...
    for (const [key, value] of Object.entries(usage)) {
      if (!PUBLIC_USAGE_KEYS.has(key)) continue
...
export class BailingHubAgentClientRuntime {
  constructor(ctx, config, getTransport) {
    this.ctx = ctx
  ...
  install() {
    ...
    const registerCommand = () => this.ctx.commands.register({
      name: 'bailinghub',
      description: 'authorize, inspect, or resync the BailingHub Agent Client connection',
      input: { hint: 'login|status|logout|workspaces|use <workspace>|sync' },
  ...
  async handleCommand(invocation) {
    ...
    const [action, ...rest] = input ? input.split(/\s+/u) : ['status']
    try {
      switch (action) {
        case 'login':
          return { kind: 'success', text: commandText(await this.login()) }
        case 'status':
        ...
        default:
          return {
            kind: 'error',
            text: 'Usage: /bailinghub login|status|logout|workspaces|use <workspace>|sync',
          }
      }
    } catch (error) {
      return {
  ...
  stateFor(agent) {
    ...
    state = createState(agent, this.defaultWorkspace, this.config.connectionName)
  ...
  onInboxClaimed({ agent, message, turn }) {
    ...
    const rawMessageId = String(
      message.id ?? messageAlias('source', state.sessionId, String(turn), visibleText(message)),
    )
    const claim = {
      id: messageAlias('user', state.sessionId, String(turn), rawMessageId),
      text: visibleText(message),
    }
    const claims = state.pendingClaims.get(turn) ?? []
    claims.push(claim)
    state.pendingClaims.set(turn, claims)
  }
    ...
    return {
      ...
      execute: async (arguments_, exec) => {
        ...
        try {
          ...
          const result = await transport.invoke(
            {
              ...
              tool: tool.name,
              arguments: cloneJson(arguments_, 'tool arguments'),
            },
            {
              workspace: state.workspace,
              connectionName: state.connectionName,
          ...
          if (isAcceptedUnknown(error)) throw acceptedUnknownFailure('business-tool', invocationId)
    ...
    return {
      ...
      execute: async (arguments_, exec) => {
        ...
        try {
          ...
          if (isAcceptedUnknown(error)) throw acceptedUnknownFailure('resume', invocationId)
  ...
  decorateAssembly(base, state, removedNames = new Set()) {
    ...
    if (!run || run.status !== 'active') {
      ...
      const reason = run?.degradedReason ?? (this.config.valid ? 'no active user turn' : 'configuration required')
  ...
  async login(options = {}) {
    return this.callConnection('login', {
      ...
      connectionName: options.connectionName ?? this.config.connectionName,
    })
  }

  async status(options = {}) {
    if (!this.config.valid) {
      return { state: 'unconfigured', missing: [...this.config.invalid] }
    }
  ...
  async use(workspace, options = {}) {
    ...
    const connectionName = options.connectionName ?? this.config.connectionName
