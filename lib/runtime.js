...
export class BailingHubAgentClientRuntime {
  ...
  async replaceAuthorizedTools(state, group, preferredNames = [], preferredRefs = []) {
    ...
    try {
      ...
      this.registerArtifactTools(state, group)
    } catch (error) {
      await this.clearActiveTools(state)
      throw error
    }
        ...
        const search = async () => {
          ...
          for (const entry of targets) {
            ...
            try {
              ...
              results.push({ authorization_ref: target.authorization.ref, state: 'available', discovery: result.discovery, candidate_update: 'replace' })
            } catch (error) {
          ...
          return {
            ...
            toolset: this.getSessionToolState(state.sessionId).toolset,
  ...
  async replaceActiveTools(state, run) {
    ...
    try {
      ...
      this.registerArtifactTools(state, run)
    } catch (error) {
      await this.clearActiveTools(state)
      throw error
    }
  ...
  registerArtifactTools(state, run) {
  ...
  registerDefinition(state, definition) {
    ...
    const registered = { ...definition, execute: async (args, exec) => {
      try { return await originalExecute(args, exec) }
      catch (error) {
      ...
      if (!record.invoked) {
        ...
        try {
          ...
          if (!isAcceptedUnknown(error)) {
            const failure = publicFailure('invoke', error, { invocationId })
  ...
  searchDefinitionFor(state, run) {
    return {
      name: 'search_business_capabilities',
      description: 'Search within the authorized BailingHub workspace and replace this session run\'s active business-tool set with up to 12 relevant candidates. This grants no new authority. ' + DISCOVERY_GUIDANCE,
      parameters: {
        type: 'object',
        properties: {
  ...
  getSessionToolState(sessionId) {
    ...
    return {
      state: availability,
      toolset: {
        update: run?.multi ? 'recompute_shared' : 'replace', scope: 'dsh_session',
        generation: state?.toolsetGeneration ?? 0, active_count: activeTools.length, limit: MAX_ACTIVE_TOOLS,
        omitted_tool_count: run?.shared?.omitted ?? 0, conflicting_tool_count: run?.shared?.conflicts.length ?? 0,
      },
  ...
  getToolDispatchFeedback(sessionId, { toolName, callId, errorCode } = {}) {
    ...
    for (const run of [state.currentRun]) {
      const binding = typeof callId === 'string' ? run.calls?.get(callId) : undefined
      if (binding?.registeredName !== toolName || !binding.invocationId) continue
    ...
    return describeFailure({ code: 'tool_not_loaded' }, { operation: 'tool_dispatch', origin: 'host', dispatch: 'not_dispatched' })
  }
