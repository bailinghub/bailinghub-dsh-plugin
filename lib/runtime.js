...
function feedbackOperation(operation) {
  ...
  if (['search', 'invoke', 'resume'].includes(operation)) return operation
...
export class BailingHubAgentClientRuntime {
  constructor(ctx, config, getTransport, recoveryOptions = {}, { scopeStore, archiveStore, artifactSource, artifactStore, invocationStore, toolLifecycle = 'active_turn' } = {}) {
  ...
  async recoverInvocationBinding(state, invocationId) {
    ...
    const run = { runId: entry.runId, turn: entry.turn, capabilityRevision: entry.capabilityRevision, invocations: new Map() }
  ...
  initializeAuthorizations(state) {
    ...
    if (entries.length === 1 && !this.sessionToolReuse) {
      ...
      if (this.sessionToolReuse) state.sessionToolCache = new SessionToolCache()
      state.authorizations = entries.map((authorization) => {
        ...
        if (state.crossSystem || this.sessionToolReuse) {
        ...
        if (this.sessionToolReuse) target.toolCacheKey = digestId(scopeFingerprint(snapshot), authorization.ref, authorization.connectionKey, authorization.sessionId)
  ...
  async startAuthorizedRuns(state, turn, signal) {
    ...
    const group = { turn, multi: true, crossSystem: state.crossSystem, sessionTools: this.sessionToolReuse,
      single: this.sessionToolReuse && state.authorizations.length === 1,
      lazy: state.crossSystem || this.sessionToolReuse, status: 'active', targets: [], calls: new Map(),
      ...(this.sessionToolReuse ? { preparedCalls: new Map() } : {}),
      userMessageId: state.pendingClaims.get(turn)?.[0]?.id,
      ...(state.crossSystem || this.sessionToolReuse ? { abortController: new AbortController() } : {}) }
  ...
  async replaceAuthorizedTools(state, group, preferredNames = [], preferredRefs = []) {
    ...
    definitions.push(this.authorizedSearchDefinition(state, group), this.authorizedResumeDefinition(state, group))
  ...
  async replaceActiveTools(state, run, preferredNames = []) {
    ...
    definitions.push(this.searchDefinitionFor(state, run), this.resumeDefinitionFor(state, run))
    ...
    const registered = { ...definition, execute: async (args, exec) => {
      ...
      try {
        ...
        const operation = definition.name === 'search_business_capabilities' ? 'search'
          : definition.name === 'resume_governed_tool_invocation' ? 'resume' : 'invoke'
  ...
  async waitForInvocation(state, record, options = {}) {
    ...
    while (!invocationIsTerminal(record.latest)) {
      ...
      if (rateWait >= remaining && record.latest) {
        const deferred = timedOutInvocationResult(record.latest, record.invocationId, attempts)
      ...
      try {
        ...
        if (isInvocationNotFound(error)) {
          ...
          try {
            ...
            throw publicFailure('resume', cancelled, { invocationId: record.invocationId })
          }
        }
        if (isInvocationNotFound(error) || !isAcceptedUnknown(error)) {
          throw publicFailure('resume', error, { invocationId: record.invocationId })
        }
      }
    }
