...
import {
  ...
  sameSystemAuthorizations, sharedDeclarations, unwrapAuthorization,
...
export class BailingHubAgentClientRuntime {
  ...
  async handleCommand(invocation) {
    ...
    // A command may authorize another identity before the first conversation.
    // New sessions must then read the registry again, not the startup snapshot.
  ...
  async onAssemble(assembly, context, next) {
    ...
    await this.initializeAuthorizations(state)
  ...
  async initializeAuthorizations(state) {
    state.authorizationInit ??= (async () => {
      if (!this.connectionReady) return
      try {
        let registry
        if (this.bootstrapRegistry) {
          registry = this.bootstrapRegistry.value
          this.bootstrapRegistry = undefined
        } else {
          registry = await this.connectionsList()
        }
        const entries = sameSystemAuthorizations(
          registry, state.binding, state.sessionId, normalizeConfig, sanitizePublicValue,
        )
        ...
        // An unavailable registry preserves the existing single-connection path.
      }
    })()
    return state.authorizationInit
  }
