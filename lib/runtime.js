...
export class BailingHubAgentClientRuntime {
  ...
  async prepareSessionArchive(sessionId) {
    ...
    if (scope.mode !== 'business' || !scope.locked) {
      this.archiveAvailability.set(sessionId, scope.mode === 'blocked' ? 'blocked' : 'inactive')
      return false
    }
    ...
    if (typeof transport.syncConversationArchive !== 'function' ||
      ...
      this.archiveAvailability.set(sessionId, 'unsupported')
      return false
    }
    ...
    await this.conversationOutbox.open(sessionId, {
      ...
      members: snapshot.authorizations.map((item) => ({
        connectionKey: item.connectionKey, workspace: item.workspace, expectedSessionId: item.sessionId, label: item.label,
      })),
    })
  ...
  async startRunOnce(state, turn, signal, registerTools = true) {
    ...
    try {
      ...
      if ((state.parentState ?? state).endedTurns.has(turn)) {
  ...
  onSessionEvent(session, event) {
    ...
    if (event.type === 'turn/end') state.endedTurns.set(turn, completionStatus(event.data?.reason))
