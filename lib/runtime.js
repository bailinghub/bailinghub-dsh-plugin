...
export class BailingHubAgentClientRuntime {
  ...
  async rememberInvocationResult(record, result) {
  ...
  async waitForInvocation(state, record, options = {}) {
    ...
    while (!invocationIsTerminal(record.latest)) {
      ...
      try {
        ...
        if (result) await this.rememberInvocationResult(record, result)
        if (options.managed) {
          this.assertLiveTargetTurn(state, options.activeTurn)
          if (options.signal?.aborted) throw abortError()
        }
    ...
    return this.runInvocationOperation(record, async () => {
      ...
      if (run.taskBinding) {
        ...
        if (rateWait) {
          ...
          await this.sessionTasks.binding(state.sessionId, { dispatch: true })
        }
        this.assertLiveTargetTurn(state, activeTurn)
        if (signal?.aborted) throw abortError()
        const transport = await this.getTransport()
  ...
  async inspectTrackedReceipt(state, record, signal) {
    ...
    const receipt = await transport.inspectInvocation(record.invocationId, this.connectionOptions(state, signal))
    const original = record.journalEntry
    if (receipt?.schema_version !== 'bailing.agent-invocation-receipt.v1' || receipt.read_only !== true || receipt.business_operation_performed !== false ||
      receipt.invocation_id !== original.id || receipt.agent_run_id !== original.runId || receipt.route !== original.target.workspace || receipt.tool !== original.tool) throw recoveryError('invocation_binding_conflict', record.invocationId)
    if (receipt.result !== null) receipt.result = invocationResult(receipt.result, record.invocationId, 'inspect')
    return receipt
  }
