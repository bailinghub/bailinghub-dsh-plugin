...
export class BailingHubAgentClientRuntime {
  constructor(ctx, config, getTransport, recoveryOptions = {}, { scopeStore, archiveStore, artifactSource, artifactStore } = {}) {
  ...
  authorizedResumeDefinition(state, group) {
    ...
    definition.execute = async (value, exec) => {
      ...
      if (!binding) throw Object.assign(new Error('This invocation is not bound to an authorization in this session'), { publicCode: 'agent_invalid_request' })
  ...
  rememberInvocationResult(record, result) {
  ...
  async waitForInvocation(state, record, options = {}) {
    ...
    while (!invocationIsTerminal(record.latest)) {
      ...
      try {
        ...
        if (result) this.rememberInvocationResult(record, result)
      } catch (error) {
  ...
  resumeDefinitionFor(state, run) {
    return {
      ...
      description: 'Recover the exact governed BailingHub invocation after an uncertain outcome, pending approval, in-progress response, or pre-dispatch rate limit. Honor retry_after_ms before resuming; never switch authorization or create a replacement invocation.',
      parameters: {
        type: 'object',
        properties: {
          invocation_id: {
