...
import { describeFailure, normalizeDiscovery, DISCOVERY_GUIDANCE } from './capability-feedback.js'
...
export class BailingHubAgentClientRuntime {
  ...
  async waitForInvocation(state, record, options = {}) {
    ...
    while (!invocationIsTerminal(record.latest)) {
      ...
      try {
        ...
        if (!isAcceptedUnknown(error)) throw publicFailure('resume', error, { invocationId: record.invocationId })
      }
    }
  ...
  resumeDefinitionFor(state, run) {
    return {
      ...
      description: 'Recover the exact governed BailingHub invocation after an uncertain outcome, pending approval, in-progress response, or pre-dispatch rate limit. Honor retry_after_ms before resuming; never switch authorization or create a replacement invocation. After reopening the same session, the host must retain its invocation store. Stored last-known states are not verified outcomes. Recovery can continue an approved original operation; it is not a read-only status check. If local recovery storage is unavailable, restore it before retrying.',
      parameters: {
        type: 'object',
        properties: {
          invocation_id: {
