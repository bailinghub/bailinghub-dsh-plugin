...
export class BailingHubAgentClientRuntime {
  ...
  registerArtifactTools(state, run) {
    ...
    this.registerDefinition(state, {
      ...
      description: 'List generated images registered by this host for this conversation. Returns opaque artifact references; never invent references or use local paths. Listing sends no files to a business system.',
          ...
          try {
            results.push(await deliverArtifact({ source: this.artifactSource, store: this.artifactStore, transport,
              ...
              metadata: { clientConversationId: messageAlias('conversation', state.sessionId), clientTurnId: messageAlias('turn', state.sessionId, String(run.turn)),
                ...(target.currentRun?.runId ? { runId: target.currentRun.runId } : {}) },
