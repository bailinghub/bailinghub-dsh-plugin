...
function doctorText(report) {
  ...
  const lines = [
    ...
    '- Credential scope: Hub + client app + workspace (connectionName is an alias)',
...
export class BailingHubAgentClientRuntime {
  ...
  install() {
    ...
    const registerCommand = () => this.ctx.commands.register({
      name: 'bailinghub',
      description: 'authorize, diagnose, inspect, or resync the BailingHub Agent Client connection',
      input: { hint: 'doctor|login|status|logout|workspaces|use <workspace>|sync' },
    ...
    try {
      switch (action) {
        ...
        default:
          return {
            kind: 'error',
            text: 'Usage: /bailinghub doctor|login|status|logout|workspaces|use <workspace>|sync',
          }
      }
    } catch (error) {
      return {
  ...
  async doctor(options = {}) {
    const report = {
      ...
      checks: {
        configuration: {
          ok: this.config.valid,
          invalid: [...this.config.invalid],
        },
