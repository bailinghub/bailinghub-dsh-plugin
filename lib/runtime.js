...
export class BailingHubAgentClientRuntime {
  constructor(ctx, config, getTransport, recoveryOptions = {}, { scopeStore, archiveStore, artifactSource, artifactStore, invocationStore } = {}) {
  ...
  getSessionToolState(sessionId) {
    ...
    const activeTools = run?.multi ? (run.shared?.tools ?? []).filter((entry) => names.includes(entry.name)).map((entry) => ({
    ...
    return {
      ...
      toolset: {
        ...
        visible_count: state?.visibleToolNames.filter(name => names.includes(name)).length ?? 0, visible_limit: VISIBLE_TOOL_LIMIT,
