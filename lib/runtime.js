...
import { SessionScopeCoordinator, scopeError } from './session-scope.js'
...
export class BailingHubAgentClientRuntime {
  constructor(ctx, config, getTransport, recoveryOptions = {}, { scopeStore } = {}) {
    this.ctx = ctx
  ...
  install() {
    ...
    const registerCommand = () => this.ctx.commands.register({
      ...
      input: { hint: 'scope [set <connection-key>...|none]|doctor|connections list|add|use|remove|login|status|logout|workspaces|use <workspace>|sync' },
    ...
    try {
      ...
      switch (action) {
        ...
        default:
          return {
            kind: 'error',
            text: 'Usage: /bailinghub scope [set <connection-key>...|none]|doctor|connections list|add|use|remove|login|status|logout|workspaces|use <workspace>|sync',
          }
      }
    } catch (error) {
      return {
  ...
  async setSessionScope(sessionId, request) {
    const existing = this.statesBySessionId.get(sessionId)
    const session = existing?.agent.session ?? this.observedSessions.get(sessionId)
    if (session?.firstLiveSeq > 0 || session?.events?.some((event) => event.type === 'turn/start' || event.type === 'user/message' && event.data?.source?.kind === 'user')) {
  ...
  observeSession(session) {
    if (session?.id) this.observedSessions.set(String(session.id), session)
  }

  getSessionScope(sessionId) { return this.sessionScopes.get(sessionId) }

  restoreSessionScope(sessionId) { return this.sessionScopes.restore(sessionId) }
  ...
  async onAssemble(assembly, context, next) {
    ...
    if (turn >= 0 && (!state.runs.has(turn) || state.startPromises.has(turn))) {
      state.scopeView = await this.sessionScopes.begin(state.sessionId, agent.session.firstLiveSeq > 0)
      if (state.scopeView.mode !== 'business') {
        ...
        state.currentRun = { turn, status: 'scope_inactive', degradedReason: state.scopeView.mode === 'chat'
          ...
          : 'session scope unavailable; start a new conversation and select authorizations' }
    ...
    if (state.currentRun?.status === 'active') {
      try { this.sessionScopes.assertCurrent(state.sessionId) }
        ...
        state.currentRun.degradedReason = 'session scope unavailable; start a new conversation and select authorizations'
  ...
  async startRun(state, turn, signal) {
    ...
    const promise = (state.authorizations
      ? this.startAuthorizedRuns(state, turn, signal)
      : this.startRunOnce(state, turn, signal)).finally(() => {
  ...
  connectionOptions(state, signal) {
  ...
  async assertAuthorization(state) {
    ...
    try {
      ...
      this.sessionScopes.invalidate(state.sessionId)
      throw publicFailure('authorization', error)
    }
    if (status?.state !== 'authorized' || status.connectionKey !== state.authorization.connectionKey ||
      status.workspace !== state.workspace || !UUID.test(status.sessionId ?? '') ||
      (state.authorizedSessionId && state.authorizedSessionId !== status.sessionId)) {
      this.sessionScopes.invalidate(state.sessionId)
      throw new Error('The original authorization is unavailable or has changed; start a new session after authorization')
    }
  ...
  async startAuthorizedRuns(state, turn, signal) {
    ...
    await this.clearActiveTools(state)
    const group = { turn, multi: true, status: 'active', targets: [], calls: new Map() }
    ...
    for (const target of state.authorizations) {
      ...
      group.targets.push({ state: target, run: target.currentRun })
    }
    try {
  ...
  assertActiveGroup(state, group, exec) {
  ...
  async replaceAuthorizedTools(state, group, preferredNames = []) {
    await this.clearActiveTools(state)
    group.shared = sharedDeclarations(group.targets.map((target) => target.state), MAX_ACTIVE_TOOLS, preferredNames)
    try {
  ...
  authorizedDefinition(state, group, shared) {
    ...
    return {
      name: tool.name,
      description: `${tool.description}\nChoose the authorized account for this call. scope=${tool.scope}, risk=${tool.risk}, approval_required=${tool.approvalRequired}, readonly=${tool.readonly}, idempotent=${tool.idempotent}.`,
      ...
      execute: async (value, exec) => {
        this.assertActiveGroup(state, group, exec)
  ...
  authorizedSearchDefinition(state, group) {
    return {
      ...
      description: 'Find shared business tools within this session\'s authorizations. Optionally restrict the search to one authorization. This grants no new authority.',
      parameters: {
      ...
      execute: async (value, exec) => {
        ...
        const search = async () => {
          ...
          const names = []
          const results = []
          for (const { state: target, run } of targets) {
            try {
          ...
          this.assertActiveGroup(state, group, exec)
          await this.replaceAuthorizedTools(state, group, names)
          return {
            authorizations: results,
            active_tools: group.shared.tools.map(({ tool, targets }) => ({
              name: tool.name, authorization_refs: targets.map((target) => target.authorization.ref),
            })),
  ...
  authorizedResumeDefinition(state, group) {
    ...
    definition.execute = async (value, exec) => {
      ...
      const currentRun = group.targets.find(({ state: target }) => target === binding.state).run
  ...
  async startRunOnce(state, turn, signal, registerTools = true) {
    const removedNames = new Set(state.knownToolNames)
    await this.clearActiveTools(state)
  ...
  decorateAssembly(base, state, removedNames = new Set()) {
    ...
    if (run.multi) {
      const directory = run.targets.map(({ state: target, run: targetRun }) => ({
        ...
        state: targetRun?.status === 'active' ? 'available' : 'unavailable',
      }))
    ...
    completion.syncing = (async () => {
      ...
      while (completion.attempts < 3) {
        ...
        try {
          const transport = await this.getTransport()
          await this.assertAuthorization(state)
          await transport.completeRun(run.runId, cloneJson(completion.payload), this.connectionOptions(state))
