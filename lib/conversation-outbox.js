...
const TARGET_SCHEMA = 'bailing.agent-conversation-outbox.v2'
...
function contextSnapshot(value) {
  const copy = structuredClone(value)
  const crossSystem = copy?.binding && !Object.hasOwn(copy.binding, 'clientAppId')
  if (typeof copy?.clientConversationId !== 'string' || !copy.clientConversationId || copy.clientConversationId.length > 256 ||
    !copy.binding || typeof copy.binding.hubUrl !== 'string' ||
    (!crossSystem && (typeof copy.binding.clientAppId !== 'string' || typeof copy.binding.workspace !== 'string')) ||
    ...
    Object.keys(copy.binding).some((key) => !(crossSystem ? ['hubUrl'] : ['hubUrl', 'clientAppId', 'workspace']).includes(key))) throw fail('ARCHIVE_MEMBERS_INVALID')
  const keys = new Set()
  ...
  for (const member of copy.members) {
    if (!KEY.test(member?.connectionKey ?? '') || keys.has(member.connectionKey) || !UUID.test(member.expectedSessionId ?? '') ||
      identities.has(member.expectedSessionId.toLowerCase()) ||
      (crossSystem ? member.hubUrl !== copy.binding.hubUrl || !/^[a-z0-9][a-z0-9_-]{1,63}$/.test(member.clientAppId ?? '') || !/^[a-z0-9][a-z0-9_-]{1,63}$/.test(member.workspace ?? '')
        : member.workspace !== copy.binding.workspace) || typeof member.label !== 'string' || member.label.length > 128 ||
      Object.keys(member).some((key) => !['connectionKey', 'workspace', 'expectedSessionId', 'label', ...(crossSystem ? ['hubUrl', 'clientAppId'] : [])].includes(key))) throw fail('ARCHIVE_MEMBERS_INVALID')
    keys.add(member.connectionKey)
    identities.add(member.expectedSessionId.toLowerCase())
  }
  return copy
}

function validateRecord(record, sessionId, context) {
  if (!record || record.schema !== (Object.hasOwn(context.binding, 'clientAppId') ? SCHEMA : TARGET_SCHEMA) || record.sessionId !== sessionId || !Number.isSafeInteger(record.revision) || record.revision < 1 ||
...
export class ConversationOutbox {
  ...
  async load(entry) {
    ...
    if (saved) entry.record = validateRecord(saved, entry.sessionId, entry.context)
      ...
      const record = {
        schema: Object.hasOwn(entry.context.binding, 'clientAppId') ? SCHEMA : TARGET_SCHEMA,
        sessionId: entry.sessionId, revision: 1, clientArchiveId: randomUUID(), context: entry.context,
  ...
  // Host receipt actions may reopen before a model turn. Read only the original
  // local outbox; do not create an archive, reconstruct text, flush or upload it.
  restoreExisting(sessionId, value) {
    const context = contextSnapshot(value)
    let entry = this.entries.get(sessionId)
    if (entry && digest(entry.context) !== digest(context)) throw fail('ARCHIVE_BINDING_CHANGED')
    if (!entry) {
      entry = { sessionId, context, pending: [], status: 'inactive' }
      this.entries.set(sessionId, entry)
    }
    return this.serialize(entry, async () => {
      try {
        if (entry.status === 'storage_error') return this.status(sessionId)
        if (!entry.record) {
          const saved = await this.store.load(sessionId)
          if (saved) entry.record = validateRecord(saved, sessionId, context)
        }
        entry.coverageLoaded = true
        entry.status = entry.pending.length ? 'storage_error' : entry.record?.syncState ?? 'inactive'
