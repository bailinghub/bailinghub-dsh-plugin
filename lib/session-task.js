...
export class SessionTaskCoordinator {
  ...
  async verify(id, taskId) {
    ...
    for (const member of scope.authorizations) {
      ...
      let caps
      try { caps = await transport.getTaskControlCapabilities(options) }
      catch (error) {
      ...
      if (!taskId) continue
      if (!caps.supported || caps.inspect_invocation !== true || typeof transport.getTask !== 'function' ||
    ...
    await this.scopes.assertUsable(id)
    return { supported, mode: required ? 'required' : 'optional', scope, snapshot, members: members.sort((a, b) => a.connectionKey.localeCompare(b.connectionKey)) }
  }
  ...
  refresh(id, restore = false) {
    return this.serial(id, async () => {
      ...
      try {
        ...
        if (restore && entry.pending && !record) { await this.verify(id, entry.pending.entries[0].taskBinding.task_id); await this.save(id, entry.pending); record = entry.record }
        if (!record && entry.intendedId) throw taskError(entry.failure ?? 'TASK_REQUIRED')
        const verified = await this.verify(id, record?.entries[0].taskBinding.task_id)
