...
export class SessionTaskCoordinator {
  ...
  async verify(id, taskId, selection) {
    if (!selection) {
    ...
    const scope = selection?.record ?? this.scopes.snapshot(id)
    ...
    if (selection) await selection.check()
    else await this.scopes.assertUsable(id)
  ...
  coordinates(id) {
    return this.serial(id, async () => {
      const selection = await this.scopes.verifiedSelection(id)
      const scope = selection.record
      if (!scope?.authorizations.length) return { state: 'inactive', availability: 'not_checked',
        reason: scope ? 'SESSION_SCOPE_CHAT_ONLY' : 'SESSION_SCOPE_UNSELECTED' }
      if (new Set(scope.authorizations.map(member => member.sessionId.toLowerCase())).size !== scope.authorizations.length) throw taskError('TASK_MEMBER_MISMATCH')
      const entry = this.entry(id)
      if (entry.failure?.startsWith('TASK_STORE_')) throw taskError(entry.failure)
      const record = await this.read(id)
      if (entry.pending || (!record && entry.intendedId)) throw taskError(entry.failure ?? 'TASK_REQUIRED')
      const verified = await this.verify(id, record?.entries[0].taskBinding.task_id, selection)
      if (!verified.supported) throw taskError('TASK_UNSUPPORTED')
      if (record) {
        const expected = { scopeHash: scopeFingerprint(scope), members: verified.members,
      ...
      // Catch a concurrent durable binding change; never write or repair sidecars here.
      if (metadataHash(await this.read(id)) !== metadataHash(record)) throw taskError('TASK_BINDING_CONFLICT')
      await selection.check()
      return { state: 'ready', availability: 'supported', sessionId: id,
        clientConversationId: this.conversationId(id), scopeRevision: scope.revision, scopeLocked: scope.locked,
        members: scope.authorizations.map(member => ({ connectionKey: member.connectionKey,
          agentSessionId: member.sessionId, clientAppId: memberBinding(scope, member).clientAppId, workspace: member.workspace })) }
    })
  }
