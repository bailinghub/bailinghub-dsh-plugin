...
const SCHEMAS = new Set(['bailing.agent-session-task.v1'])
...
function storeError(code = 'TASK_STORE_UNAVAILABLE') {
  const error = new Error(code === 'TASK_STORE_CONFLICT'
    ? 'The task binding changed or is being saved. Read it again before retrying.'
    : 'The task binding store is unavailable. Recovery metadata was not safely confirmed.')
  error.code = code
  return error
}

function safeError(error) {
  return ['TASK_STORE_CONFLICT', 'TASK_STORE_UNAVAILABLE'].includes(error?.code)
    ? error : storeError()
}
...
function assertRevision(current, expectedRevision) {
  if ((current?.revision ?? null) !== expectedRevision) throw storeError('TASK_STORE_CONFLICT')
}
...
function defaultDirectory() {
  ...
  return join(resolve(expanded), 'plugins', 'dsh-bailinghub', 'session-tasks')
}
...
async function acquireLock(path) {
  ...
  while (true) {
    ...
    try {
      ...
      if (performance.now() >= deadline) throw storeError('TASK_STORE_CONFLICT')
...
export function createFileSessionTaskStore({ directory } = {}) {
...
export function createMemorySessionTaskStore() {
