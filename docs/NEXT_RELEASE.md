# Next plugin release: check stock, then update and list a shop product

[简体中文](#简体中文)

**Unreleased source.** The planned pairing is Core 0.7.0, SDK 0.5.0 and DSH 0.5.0, subject to release checks. Published Core 0.6.1 and SDK/DSH 0.4.0 do not include these additions. The current plugin lockfile still installs SDK 0.4.0; cloning this plugin alone does not enable the full new flow.
...
The 0.4 plugin already supports multiple accounts in one system. The next plugin extends that flow to different selected systems. It looks up stock with inventory tools, then performs permitted shop actions with shop tools. If only the shop is needed, inventory is not marked as executed. The original permissions and approvals apply to each action.
...
## Who needs to prepare an upgrade
...
1. Prepare the matching Core and apply its outstanding migrations, including 058/059 for this combined change. Installing the plugin does not deploy or migrate the Hub.
2. Release the SDK before changing the plugin to an exact new SDK dependency and regenerating the lockfile from the registry. Until those packages exist, use matched source revisions for development only; normal 0.4.0 installation remains the stable same-system flow.
...
See [cross-system integration](CROSS_SYSTEM_CONVERSATIONS.md), [host contract](AGENT_CLIENT_CONTRACT.md), [compatibility](COMPATIBILITY.md), [release procedure](RELEASING.md) and [Core scenarios](https://github.com/bailinghub/bailinghub/blob/main/docs/RELEASE_NOTES_NEXT.en.md).

## 简体中文

**这是未发布源码说明。** 计划配套 Core 0.7.0、SDK 0.5.0、DSH 0.5.0，须经发布检查。公开 Core 0.6.1 与 SDK/DSH 0.4.0 不含这些增量。插件当前锁文件仍安装 SDK 0.4.0，单独克隆插件不能启用完整新流程。
...
先准备 Core 并按账本执行未应用迁移，本次组合包括 058/059。正式发布先 SDK，待其可公开安装后，再改插件精确依赖并从注册表生成锁文件。此前仅以匹配源码联调，普通 0.4.0 安装继续对应稳定的同系统流程。
...
详见[跨系统接入](CROSS_SYSTEM_CONVERSATIONS.md#简体中文)、[宿主契约](AGENT_CLIENT_CONTRACT.md)、[兼容说明](COMPATIBILITY.md)、[发布流程](RELEASING.md)和[Core 场景说明](https://github.com/bailinghub/bailinghub/blob/main/docs/RELEASE_NOTES_NEXT.md)。
