...
离线重开导致范围暂时 `blocked` 时，联网后可在**同一 runtime、同一 Session** 再次调用
`restoreSessionScope` 或 `syncSessionArchive`。全部原授权核验通过后才能恢复，补传只上传原事件，
不重新发送用户消息或执行业务操作。明确撤销、身份变化或存储冲突仍阻断整个范围。
本地 `storage_error`、未保存事件及已知历史缺口不会被网络阻断状态覆盖。
