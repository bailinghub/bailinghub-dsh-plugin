...
用户先说“查一下这个商品”，下一条再说“看它的详情”，宿主可以保留同一会话里最近发现的完整工具声明。
下一轮按需取得当前目标的新上下文和新执行记录；声明仍有效时，不必再发一次能力搜索请求。
同轮在商城操作和库存查询之间往返，也不需要每次重新搜索。普通聊天不会为缓存预建业务运行记录。

这是尚未发布的精确候选：由宿主接入方显式设置 `toolLifecycle: 'session'`，普通用户无需配置。
64 个缓存条目、12 个完整参数展示窗口是工具种类预算，不是操作次数限制。
准备成功并不表示商品已修改；已发出但结果未知的操作仍须沿原调用恢复。
接入前请阅读[完整生命周期与宿主契约](SESSION_TOOL_REUSE.md)，按候选清单核对包哈希。
...
- 只有超过缓存上限被卸载、声明变化、轮次结束或需要新能力时才重新发现；未确认的写操作始终恢复原调用。
...
这部分位于未发布候选中，不能仅按与公开包相同的版本号判断已安装。
