...
> **未发布候选：**当前源码新增“同一个中枢下，多个业务系统共用一个会话”。需要配套的 Core、SDK
> 与插件候选，仅安装公开的 0.4.0 包不会获得此能力。先看[使用与接入说明](CROSS_SYSTEM_CONVERSATIONS.md#简体中文)。
> 下方公开版本安装说明仍对应已经发布的同系统多授权能力。

本候选还会在首次搜索工具前，让助手看到管理员维护的系统用途说明，分清本会话所选目标分别负责什么。
介绍不会增加权限；具体能执行哪些动作，仍由各授权的能力查询和业务规则决定。

配套的授权主体展示候选还会读取业务后端提供的主体名称，例如组织、账号、项目或部门，客户端可在授权成功后
自动显示。它和“产品负责什么”的系统说明分别维护。名称缺失时显示“授权名称待同步”；同名或改名不会合并、
重建授权，也不会改写原会话记录。详见[授权主体展示契约](AGENT_CLIENT_CONTRACT.md#unreleased-authorization-subject-display)。
...
以下是公开 0.4.0 的本机标签流程；配套展示候选会优先使用业务提供的名称描述当前授权，同时保留内部连接标识。
