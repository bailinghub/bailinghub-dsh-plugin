...
> **未发布源码：**例如“先查保温杯库存，再把商城对应商品改为 59 元并上架”，新插件可在同一中枢内使用商城与库存各自的授权，先理解系统用途，并显示业务提供的授权对象名称。每项动作仍需对应能力和审批，商品映射须已确认。
> 公开 Core 0.6.1 / SDK 与插件 0.4.0 不包含这些增量。当前锁文件仍保留已发布 SDK，单独克隆插件不足以启用完整新能力。
> [看变化与升级准备](NEXT_RELEASE.md#简体中文)。下方安装说明仍对应稳定的同系统多授权版本。
