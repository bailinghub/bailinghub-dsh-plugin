...
test('real host preserves empty, complete, capped and legacy-unknown discovery semantics', async (t) => {
  ...
  for (const [count, total] of [[0, 0], [3, 3], [12, 30]]) {
    ...
    assert.equal(response.value.toolset.update, 'replace')
  ...
  assert.equal(legacy.value.active_tools.length, 2)
  const before = f.mock.calls.length
  assert.equal(f.runtime.getSessionToolState(f.session.id).toolset.active_count, 2)
  assert.equal(f.mock.calls.length, before, 'host inspection makes no requests')
})

test('cross-system shared limit separates target counts and stale registry calls never invoke business', async (t) => {
  ...
  assert.equal(b.value.toolset.active_count, 12)
  assert.equal(f.runtime.inspectSession(f.session.id).activeToolCount, 12)
  assert.equal(b.value.toolset.omitted_tool_count, 4)
  assert.equal(b.value.omitted_tool_count, 4)
  assert.equal(b.value.toolset.update, 'recompute_shared')
  assert.ok(b.value.toolset.generation > a.value.toolset.generation)
  assert.equal(b.value.active_tools.some((entry) => entry.name === oldName), false)
  const failed = await f.execute(oldName, { authorization_ref: f.refs[keys[0]], arguments: {} })
  assert.equal(failed.isError, true)
  assert.equal(failed.error.info.code, 'UNKNOWN_TOOL')
  assert.equal(failed.meta.bailinghub.feedback.category, 'tool_not_loaded')
  assert.equal(failed.meta.bailinghub.feedback.dispatch, 'not_dispatched')
  assert.equal(JSON.parse(failed.content[0].text).feedback.next_action, 'rediscover')
  ...
  assert.equal(f.runtime.getToolDispatchFeedback(f.session.id, { toolName: b.value.active_tools[0].name, errorCode: 'UNKNOWN_TOOL' }), null)
  const restored = await f.search(f.refs[keys[0]])
  const loaded = restored.value.active_tools.find((entry) => entry.name === oldName)
  assert.ok(loaded)
  assert.equal((await f.execute(loaded.name, { authorization_ref: f.refs[keys[0]], arguments: {} })).isError, false)
...
test('multi-target search preserves per-target failure reasons instead of only unavailable', async (t) => {
  ...
  assert.equal(failed.value.toolset.active_count, 0)
