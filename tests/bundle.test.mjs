...
test('is a publishable stable native Cordis plugin', () => {
  ...
  assert.equal(packageJson.dependencies['bailinghub-mcp-server'], '0.5.0')
...
test('requires every publishable package to install one exact SDK version', () => {
  ...
  const publicLock = {
    packages: {
      '': { dependencies: { 'bailinghub-mcp-server': '0.4.0' } },
      'node_modules/bailinghub-mcp-server': { version: '0.4.0' },
    },
  }
...
test('rejects public packages with ranged, optional-peer, optional, or local SDK dependencies', () => {
  ...
  assert.throws(
    () => validateReleaseDependencyContract({
      ...publicStable,
      dependencies: { ...publicStable.dependencies, 'bailinghub-mcp-server': '^0.4.0' },
    }),
  ...
  assert.throws(
    () => validateReleaseDependencyContract({
      ...publicStable,
      dependencies: { ...publicStable.dependencies, 'bailinghub-mcp-server': '0.4.0-rc.1' },
  ...
  assert.throws(
    () => validateReleaseDependencyContract({
      ...publicStable,
      dependencies: { ...publicStable.dependencies, 'bailinghub-mcp-server': '0.4.0' },
      peerDependencies: { 'bailinghub-mcp-server': '>=0.4.0' },
  ...
  assert.throws(
    () => validateReleaseDependencyContract({
      ...
      peerDependencies: undefined,
      peerDependenciesMeta: undefined,
      optionalDependencies: { 'bailinghub-mcp-server': '0.4.0' },
    }, {
      packages: {
        '': { dependencies: { 'bailinghub-mcp-server': '0.4.0' } },
        'node_modules/bailinghub-mcp-server': { version: '0.4.0' },
...
test('keeps the current stable dependency contract valid', () => {
  const matchingLock = {
    packages: {
      '': { dependencies: { 'bailinghub-mcp-server': '0.5.0' } },
      'node_modules/bailinghub-mcp-server': { version: '0.5.0' },
    },
  }
